 ...
 */
export function initFilter(type, existing) {
    const base = {
        type,
    ...
    switch (type) {
        case 'text': {
            const defaults = { type, value: '', operator: 'contains' };
            return existing?.type === 'text' ? { ...defaults, ...existing } : defaults;
        }
        case 'multi-select': {
            const defaults = { type, value: [], operator: 'in' };
            return existing?.type === 'multi-select' ? { ...defaults, ...existing } : defaults;
        }
        case 'number-range': {
            const defaults = { type, operator: 'between', value: null, min: null, max: null };
            return existing?.type === 'number-range' ? { ...defaults, ...existing } : defaults;
        }
        case 'date-range': {
            const defaults = { type, operator: 'between', value: null, start: null, end: null };
            return existing?.type === 'date-range' ? { ...defaults, ...existing } : defaults;
        }
        case 'custom':
            const defaults = { type, value: null, operator: base.operator ?? 'equals', custom: '', filterFn: undefined };
 ...
 */
export function hasValue(filter) {
    switch (filter.type) {
        case 'text':
            if (!filter.operator)
                return false;
...
export function applyColumnFilter(row, col, filter) {
    if (!col.field)
        return true;
    switch (filter.type) {
        case 'text': {
            ...
            if (!search && !['empty', 'notEmpty'].includes(filter.operator ?? ''))
                return true;
        ...
        case 'number-range': {
            ...
            switch (filter.operator) {
                case 'between':
                    if (filter.min != null && num < filter.min)
                        return false;
                    if (filter.max != null && num > filter.max)
                        return false;
                    return true;
                case 'equals':
