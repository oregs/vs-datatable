...
export function useStickyColumns(tableRef, columns, hasGroups = ref(false)) {
    ...
    const updateShadows = () => {
        ...
        const leftStickyCells = Array.from(table.querySelectorAll('.vs-sticky-left'));
        const rightStickyCells = Array.from(table.querySelectorAll('.vs-sticky-right'));
    ...
    const hasStickyColumns = (columns) => {
    ...
    const getStickyIndexes = (bodyCells, fieldToIndex, columns) => {
        const leftIndexes = [];
        const rightIndexes = [];
    ...
    const calculateOffsets = (bodyCells, indexes, direction) => {
    ...
    const createGroupChildFieldMap = (columns) => {
        const firstChildMap = new Map();
    ...
    const resolveHeaderFieldIndex = (field, fieldToIndex, groupChildField, direction) => {
        if (!field)
            return undefined;
    ...
    const applyStickyStyles = (rows, stickyIndexes, offsets, direction, fieldToIndex, groupChildField) => {
        if (isApplyingStyles)
            return;
        ...
        try {
            rows.forEach(row => {
                ...
                cells.forEach(cell => {
                    ...
                    if (resolvedIndex === undefined || !stickyIndexes.includes(resolvedIndex))
                        return;
