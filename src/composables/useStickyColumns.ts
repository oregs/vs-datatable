...
export function useStickyColumns(
  ...
  const updateShadows = () => {
    ...
    const leftStickyCells = Array.from(table.querySelectorAll('.vs-sticky-left')) as HTMLElement[]
    const rightStickyCells = Array.from(table.querySelectorAll('.vs-sticky-right')) as HTMLElement[]
  ...
  const hasStickyColumns = (columns: Column[]): boolean => {
    for (const col of columns) {
      if (col.children?.length) {
        // Check child columns
        if (
          col.children.some((child: Column) => child.sticky === 'left' || child.sticky === 'right')
        ) {
          return true
        }
      } else {
  ...
  const getStickyIndexes = (
    ...
    if (hasManualStickyColumns) {
      bodyCells.forEach((cell, index) => {
        if (
          cell.classList.contains('vs-expand-column') ||
    ...
    columns.forEach((col) => {
      if (col.children?.length) {
        col.children.forEach((child: Column) => {
  ...
  const calculateOffsets = (bodyCells: HTMLElement[], indexes: number[]): Map<number, number> => {
  ...
  const createGroupChildFieldMap = (
    columns: Column[],
  ): {
    ...
    try {
      rows.forEach((row) => {
        ...
        cells.forEach((cell) => {
          ...
          const resolvedIndex = resolveHeaderFieldIndex(
            field,
            fieldToIndex,
            groupChildField,
            direction,
  ...
  const setupSticky = async () => {
    ...
    if (stickyIndexes.left.length > 0 || stickyIndexes.right.length > 0) {
      const leftOffsets = calculateOffsets(bodyCells, stickyIndexes.left)
      const rightOffsets = calculateOffsets(bodyCells, stickyIndexes.right)
