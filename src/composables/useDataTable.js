...
export function useDataTable(props, emit, options = {}, internalRows) {
    ...
    // --- Column filters
    const { filters, filteredData, setFilter, clearFilter } = useColumnFilter(computed(() => unref(rowsRef)), getFlatColumns(unref(props.columns)), {
    ...
    // --- Pagination helpers
    const { totalRecords, recordRange, handlePageChange } = useDataTablePagination(props, emit, page, rowsPerPage, computed(() => processedRows.value));
    // --- Row Per Page
